<template>
  <div>
    ...
    <grid-panel v-show="cellType == 'grid'" :graph="gridcell">
      <detail-form type="grid" :read-only="readOnly" />
      <slot name="grid"> </slot>
    </grid-panel>
    <node-panel
      v-show="cellType == 'node'"
      :cell="nodecell"
      :formModel="nodeFormModel"
    >
      <detail-form type="node" :read-only="readOnly" />
      <slot name="node"> </slot>
    </node-panel>
    <edge-panel
      v-show="cellType == 'edge'"
      :cell="edgecell"
      :formModel="edgeFormModel"
    >
      ...
      graph.on("cell:click", ({ cell }) => {
        if (cell.isNode()) {
          ...
          this.nodeFormModel = {
            ...
            backColor: cell.attrs?.body?.fill
              ? cell.attrs?.body?.fill
              : "#D9D9D9",
            data: cell.getData()?.ports,
          };
          this.$emit("itemSelectedFunc", {
            type: "node",
            item: cell,
          ...
          this.edgeFormModel = {
            ...
            stroke: cell.getAttrs().line.stroke
              ? cell.getAttrs().line.stroke
              : "#5F95FF",
            targetMarker: targetMName,
            sourceMarker: sourceMName,
            strokeWidth: cell.getAttrs().line.strokeWidth
              ? cell.getAttrs().line.strokeWidth
              : 1,
            label:
              cell.getLabels().length > 0
