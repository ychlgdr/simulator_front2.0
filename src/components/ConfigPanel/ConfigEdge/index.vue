<template>
  <div>
    <div style="background-color: rgb(218, 218, 218); height: 40px">
    ...
    <a-form readonly>
      ...
      <a-form-item label="起点：">
        <a-select
          ...
          v-model="formModel.sourceMarker"
        >
      ...
      <a-form-item label="终点：">
        <a-select
          :disabled="readOnly"
          @change="onEndChange"
          v-model="formModel.targetMarker"
        >
          <a-select-option value="block">block</a-select-option>
          <a-select-option value="classic">classic</a-select-option>
          <a-select-option value="diamond">diamond</a-select-option>
      ...
      <a-form-item>
        <a-input placeholder="source port" v-model="sourcePort" />
        <a-input placeholder="target port" v-model="targetPort" />
...
export default {
  ...
  watch: {
    formModel(newVal, oldVal) {
      this.sourcePort = newVal.data.sourcePort;
      this.targetPort = newVal.data.targetPort;
    },
  },
  methods: {
    save() {
